...
`contributions.service_data` 支持两类后台采集器：声明式 HTTPS JSON URL，以及 `component_adapter.api_version=component-v1`。组件适配器分别声明 `collector`、`normalizer` 和对象配置；Worker 在同一个有界 Store 中依次调用 `collect` 与 `normalize`，两阶段共享 fuel、内存、HTTP body 和总超时预算。宿主按端点和已发布插件 revision 持久化 last-good、最近尝试、下次尝试、连续失败数及固定错误码，多 Worker 通过短租约 CAS 协作。失败采用最多五分钟的指数退避，Operator 请求直接读取当前 revision 的持久快照或 manifest fallback。采集延迟与来源故障因此与推理链路完全解耦。组件 HTTP/KV 调用继续遵循 manifest capability；返回数据还要通过 endpoint 的对象根 `response_schema`。服务数据专用组件使用 WIT `service-data-plugin` world；同时提供流量/Provider 能力的组件使用 `combined-plugin` authoring world。
