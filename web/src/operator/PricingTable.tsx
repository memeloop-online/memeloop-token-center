...
export function PricingTable({ rows, currency, loading, catalogFailed = false, usageLoading, usageFailed }: {
  rows: PricingRow[]; currency: string; loading: boolean; catalogFailed?: boolean; usageLoading: boolean; usageFailed: boolean;
}) {
  ...
  const filtered = rows.filter(row => (!query || `${row.model} ${row.tier?.source ?? ''}`.toLocaleLowerCase(locale).includes(query))
    && (filter === 'all' || filter === 'missing' && usageReady && !loading && !catalogFailed && !row.tier || filter === 'used' && usageReady && (row.usage?.calls ?? 0) > 0));
  ...
  return <section className="pricing-catalog">
    <div className="pricing-list-controls">
      ...
      <label>{t('pricing.filter')}<Select aria-label={t('pricing.filter')} value={filter} onChange={event => setFilter(event.target.value)}>
        <option value="all">{t('common.all')}</option><option value="used" disabled={!usageReady}>{t('pricing.filterUsed')}</option><option value="missing" disabled={!usageReady || catalogFailed}>{t('pricing.filterMissing')}</option>
      </Select></label>
      {(search || filter !== 'all') && <Button appearance="subtle" onClick={() => { setSearch(''); setFilter('all'); }}>{t('pricing.clearFilters')}</Button>}
    </div>
    <p className="pricing-table-caption" role="status">{t('pricing.tableUnit', { currency })} · {t('pricing.visibleRows', { count: formatNumber(filtered.length, locale) })}{loading ? ` · ${t('pricing.loadingPrices')}` : catalogFailed ? ` · ${t('pricing.catalogUnavailable')}` : ''}</p>
    <div className="table-scroll token-pricing-scroll" tabIndex={0} role="region" aria-label={t('pricing.title')}>
      <table className="token-pricing-table"><thead><tr><th>{t('pricing.model')}</th><th>{t('pricing.calls')}</th><th>{t('pricing.inputShort')}</th><th>{t('pricing.cachedShort')}</th><th>{t('pricing.writeShort')}</th><th>{t('pricing.outputShort')}</th><th>{t('pricing.source')}</th></tr></thead>
        <tbody>{filtered.map(row => <tr key={`${row.model}-${row.tier?.service_tier ?? 'missing'}`}>
          ...
          <td>{row.tier ? <><PriceSource source={row.tier.source} /><DetailTooltip content={`${t('pricing.updated')}: ${new Date(row.tier.updated_at).toLocaleString(locale)}`}><time tabIndex={0} dateTime={new Date(row.tier.updated_at).toISOString()}>{new Date(row.tier.updated_at).toLocaleDateString(locale)}</time></DetailTooltip></> : <span>{loading ? t('pricing.loadingPrices') : catalogFailed ? t('pricing.priceUnavailable') : t('pricing.missing')}</span>}</td>
        </tr>)}</tbody>
      </table>
      {!filtered.length && <div className="empty" role="status">{catalogFailed ? t('pricing.catalogUnavailable') : unavailableFilter ? t(usageLoading ? 'pricing.usageLoading' : filter === 'used' ? 'pricing.usedFilterUnavailable' : 'pricing.missingFilterUnavailable') : loading ? t('pricing.loadingPrices') : search || filter !== 'all' ? t('pricing.noMatches') : t('pricing.noPricesForCurrency', { currency })}</div>}
    </div>
  </section>;
}
