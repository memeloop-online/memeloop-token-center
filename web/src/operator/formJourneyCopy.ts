...
export function formJourneyCopy(locale: string) {
  return locale.startsWith('zh') ? {
    ...
    codexProxy: '此地址用于标识服务协议端点。所有网络连接均使用此账号独立的 socks5h 代理，并在代理端解析域名；继续前需要有效代理。',
    draftNoRoutes: '尚未授权模型访问；可创建后再配置路由。',
    credentialFlowHint: '填写使用者，选择可访问的模型，再按需设置预算。',
    concurrentDraftPreserved: '配置已被其他人修改。你的草稿已保留；请关闭并重新打开编辑，查看最新配置后重试。',
    ...
    codexProxy: 'This is the service protocol endpoint. All connections use this account’s separate socks5h proxy with remote DNS. A valid proxy is required to continue.',
