...
function CredentialWorkspace({ token, tenant, createSchema, policySchema }: { token: string; tenant: string; createSchema?: Record<string, unknown>; policySchema?: Record<string, unknown> }) {
  ...
  const groupFilteredValues = values.filter((value) => {
  ...
  const statusFilter = useResourceListStatusFilter('credentials', tenant, groupFilteredValues, (value) => (value.status ?? 'active') === 'active');
  ...
  return <><WriteScopeNotice tenant={tenant} />{secret && <OneTimeSecret value={secret} message={t('credentials.oneTimeSecret')} />}<section className="management-layout">
    <article className="panel"><div className="panel-title"><div><h2>{t('credentials.title')}</h2><p className="muted">{t('credentials.description')}</p></div><ResourceListStatusFilterControl filter={statusFilter} inactiveLabel={t('resourceList.inactive')} /></div>
      ...
      <div className="account-list">{statusFilter.values.length === 0 && <ResourceListStatusEmpty totalCount={statusFilter.totalCount} normalLabel={t('status.active')} empty={values.length === 0 ? t('credentials.empty') : t('credentials.noGroupResults')} />}{statusFilter.values.map((value) => {
        const memberships = credentialGroups.groups.filter((group) => group.member_ids.includes(value.key_id));
