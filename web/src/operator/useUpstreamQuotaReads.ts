...
import { upstreamQuotaPath, type UpstreamQuotaSnapshot } from './upstreamQuota';
...
export function useUpstreamQuotaReads(token: string, tenant: string, accounts: QuotaReadAccount[]) {
  ...
  function read(account: QuotaReadAccount): Promise<void> {
    ...
    const owns = () => current.current.scope === scope && current.current.accounts.some(value => value.id === account.id && value.credential_generation === generation && (value.tenant_external_id ?? tenant) === accountTenant);
  ...
  async function readAll() {
    ...
    if (owns()) { batch.current = undefined; setProgress({ done, total: selected.length, busy: false }); }
  }
