  ...
  if (props.fieldPathId.path.at(-1) === 'config') {
    if (props.registry.formContext?.providerEdit) {
      ...
      return <div className="upstream-form-sections">
        ...
        {advanced.length > 0 && <AdvancedFormSection title={zh ? '高级配置与模型预留' : 'Advanced settings and model reservations'} description={zh ? '网络范围、模型预留及扩展配置。现有字段完整保留；仅在明确了解影响时修改。' : 'Network scope, model reservations and extensions. Existing fields are preserved; change only when you understand their effects.'} invalid={advanced.some(field => Boolean(props.errorSchema?.[field.name]))}>{advanced.map(field => field.content)}</AdvancedFormSection>}
      </div>;
    }
