...
export function Operator({ route, onRouteChange, onPluginNavigation, embedded = false, showNavigation = true }: OperatorProps = {}) {
  ...
  const content = <>
    ...
    {scope.activeCredential && (scope.tenants.length > 1 || activeRoute === 'credentials') && <div className="tenant-scope-switcher"><label className="tenant-picker"><span>{t('operator.tenant')}</span><select value={scope.tenant} onChange={(event) => scope.setTenant(event.target.value)}>{scope.tenants.map((value) => <option key={value.external_id} value={value.external_id}>{value.external_id === 'default' ? t('operator.defaultTenant') : value.external_id}</option>)}</select></label></div>}
