  ...
  if (scope.activeCredential && scope.validated && scope.tenant) {
    ...
    if (isOperatorRouteKey(activeRoute)) {
      switch (activeRoute) {
        ...
        case 'sessions': page = <SessionsPage {...pageProps} focus={sessionFocus} sessionEvents={stream.sessionEvents} streamState={stream.state} streamError={stream.error} onOpenRequests={() => navigate('requests')} />; break;
