...
function AuthorizationConnection({ token, tenant, providers, existing, onChanged }: { token: string; tenant: string; providers: ProviderType[]; existing?: UpstreamAccount; onChanged: () => Promise<void> }) {
  ...
  const existingMode = existing?.connection_method === 'subscription_bridge' ? 'subscription' : providers.find((provider) => provider.id === existing?.driver)?.oauth_adapter ? 'plugin-adapter' : 'cursor-direct';
  const [mode, setMode] = useState<'subscription' | 'cursor-direct' | 'plugin-adapter'>(existing ? existingMode : 'subscription');
  const [subscriptionProvider, setSubscriptionProvider] = useState<'copilot' | 'cursor'>(existing?.config.provider === 'cursor' ? 'cursor' : 'copilot');
  const [driver, setDriver] = useState(existing?.driver ?? adapterProviders[0]?.id ?? '');
  const adapter = adapterProviders.find((provider) => provider.id === driver) ?? adapterProviders[0];
  const [name, setName] = useState(existing?.name ?? 'copilot-primary');
  const [baseUrl, setBaseUrl] = useState(typeof existing?.config.base_url === 'string' ? existing.config.base_url : 'http://cpa-subscription-bridge:8080');
  ...
  const start = async (providerConfig?: unknown) => {
    ...
    try {
      ...
      if (mode === 'subscription') setSession(await api('/internal/v1/oauth/subscription-bridge/start', token, { method: 'POST', body: JSON.stringify({ tenant_external_id: tenant, account_name: name, provider: subscriptionProvider, base_url: baseUrl, ...(bridgeSecret ? { bridge_secret: bridgeSecret } : {}), ...target }) }));
      else if (mode === 'cursor-direct') setSession(await api('/internal/v1/oauth/cursor/start', token, { method: 'POST', body: JSON.stringify({ tenant_external_id: tenant, account_name: name, provider_driver: existing?.driver ?? 'http-json', provider_config: existing?.config ?? { base_url: baseUrl }, ...target }) }));
      else if (adapter) setSession(await api('/internal/v1/oauth/provider-adapter/start', token, { method: 'POST', body: JSON.stringify({ tenant_external_id: tenant, account_name: name, provider_driver: adapter.id, provider_config: existing?.config ?? providerConfig, ...target }) }));
      setMessage(''); setError('');
    } catch (reason) { setError(messageOf(reason, t('common.requestFailed'))); }
  };
  const poll = async () => {
  ...
  return <div className="authorization-form"><p className="muted">{t('providers.oauthSecurity')}</p>
    {error && <div className="notice error" role="alert">{error}</div>}
    <label>{t('providers.method')}<select disabled={Boolean(existing)} value={mode} onChange={(event) => { const next = event.target.value as typeof mode; setMode(next); reset(); if (next === 'subscription') { setName(`${subscriptionProvider}-primary`); setBaseUrl('http://cpa-subscription-bridge:8080'); } else if (next === 'cursor-direct') { setName('cursor-primary'); setBaseUrl('http://cursor-adapter:8080'); } else if (adapter) setName(`${adapter.id}-primary`); }}><option value="subscription">{t('providers.subscription')}</option><option value="cursor-direct">{t('providers.cursorDirect')}</option>{adapterProviders.length > 0 && <option value="plugin-adapter">{t('providers.pluginAdapter')}</option>}</select></label>
    {mode === 'subscription' && <label>{t('providers.subscriptionProvider')}<select disabled={Boolean(existing)} value={subscriptionProvider} onChange={(event) => { const next = event.target.value as typeof subscriptionProvider; setSubscriptionProvider(next); setName(`${next}-primary`); reset(); }}><option value="copilot">GitHub Copilot</option><option value="cursor">Cursor</option></select></label>}
    {mode === 'plugin-adapter' && adapter && <label>{t('providers.provider')}<select disabled={Boolean(existing)} value={adapter.id} onChange={(event) => { const next = event.target.value; setDriver(next); setName(`${next}-primary`); reset(); }}>{adapterProviders.map((value) => <option key={value.id} value={value.id}>{value.display_name} · {value.source}</option>)}</select></label>}
    ...
    {mode !== 'plugin-adapter' && <label>{mode === 'subscription' ? t('providers.bridgeUrl') : t('providers.adapterUrl')}<input readOnly={Boolean(existing)} value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} /></label>}
