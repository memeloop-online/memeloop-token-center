...
function AuthorizationConnection({ token, tenant, providers, existing, onChanged }: { token: string; tenant: string; providers: ProviderType[]; existing?: UpstreamAccount; onChanged: () => Promise<void> }) {
  ...
  const start = async (providerConfig?: unknown) => {
    ...
    try {
      ...
      if (!selectedProvider) setSession(await api('/internal/v1/oauth/cursor/start', token, { method: 'POST', body: JSON.stringify({ tenant_external_id: tenant, account_name: name, provider_driver: existing?.driver ?? 'http-json', provider_config: existing?.config ?? { base_url: baseUrl }, ...target }) }));
  ...
  const poll = async () => {
    if (!session) return;
    const path = selectedProvider ? '/internal/v1/oauth/provider-adapter/poll' : '/internal/v1/oauth/cursor/poll';
    try {
