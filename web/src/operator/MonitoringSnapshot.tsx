  ...
  return <section className="operator-monitoring" aria-labelledby="monitoring-heading">
    <article className="panel">
      ...
      <section className="metrics operator-monitoring-metrics monitoring-metrics-grid" aria-label={t('monitoring.summary')}>
        <AnalyticsMetric timestamps={points.map(point => point.bucket_start)} timeZone="UTC" label={t('usage.requests')} value={count(summary.requests).text} title={count(summary.requests).title} trend={points.map((point) => point.requests)} />
        <AnalyticsMetric timestamps={points.map(point => point.bucket_start)} timeZone="UTC" label={t('traffic.success')} value={count(summary.successful_requests).text} title={count(summary.successful_requests).title} tone="positive" trend={points.map((point) => point.success)} ratio={successRate} />
        <AnalyticsMetric timestamps={points.map(point => point.bucket_start)} timeZone="UTC" label={t('traffic.failure')} value={count(summary.failed_requests).text} title={count(summary.failed_requests).title} tone="negative" trend={points.map((point) => point.failed)} ratio={successRate === null ? null : 1 - successRate} />
        <AnalyticsMetric timestamps={points.map(point => point.bucket_start)} timeZone="UTC" label={t('usage.successRate')} value={formatPercent(successRate, locale)} tone="positive" ratio={successRate} />
        <AnalyticsMetric timestamps={points.map(point => point.bucket_start)} timeZone="UTC" label={t('usage.average')} value={average.text} title={average.title} formatSample={value => analyticsDuration(value, locale).title ?? analyticsDuration(value, locale).text} trend={points.map((point) => point.avg_duration_ms)} />
        <AnalyticsMetric timestamps={points.map(point => point.bucket_start)} timeZone="UTC" label={t('usage.p95Approx')} value={p95.text} title={p95.title} formatSample={(_value, index) => { const point = points[index]; const display = histogramP95(point.p95_duration_ms, point.p95_is_capped, locale); return `${display.text} ${display.title ?? ''}`; }} trend={finiteP95Points(points).map((point) => point.p95_duration_ms)} />
        <AnalyticsMetric timestamps={points.map(point => point.bucket_start)} timeZone="UTC" label={t('traffic.cost')} value={<CostLines costs={summary.costs} />} formatSample={(_value, index) => { const cost = points[index].costs.find(item => item.currency === currency); return cost ? formatCurrencyDisplay(cost.cost, cost.currency, locale).title ?? '—' : '—'; }} trend={currency ? points.map((point) => point.costs.some(cost => cost.currency === currency) ? Number(point.costs.find(cost => cost.currency === currency)!.cost) : null) : undefined} />
        <AnalyticsMetric timestamps={points.map(point => point.bucket_start)} timeZone="UTC" label={t('monitoring.freshness')} value={<Freshness snapshot={snapshot} />} />
      </section>
    </article>
