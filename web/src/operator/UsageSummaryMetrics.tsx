...
export function UsageSummaryMetrics({ stats }: { stats: OperatorUsageAnalysis }) {
  ...
  const numeric = (label: string, value: number, trend: number[], tone = '') => <AnalyticsMetric timestamps={points.map(point => point.bucket_start)} timeZone="UTC" label={label} value={<span className="metric-number"><span className="metric-exact" title={number(value).title}>{number(value).text}</span></span>} trend={trend} tone={tone} />;
