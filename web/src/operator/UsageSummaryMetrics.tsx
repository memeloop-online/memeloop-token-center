...
export function UsageSummaryMetrics({ stats }: { stats: OperatorUsageAnalysis }) {
  ...
  const numeric = (label: string, value: number, trend: number[], tone = '') => <AnalyticsMetric label={label} value={<span className="metric-number"><span className="metric-exact" title={number(value).title}>{number(value).text}</span></span>} trend={trend} tone={tone} />;
