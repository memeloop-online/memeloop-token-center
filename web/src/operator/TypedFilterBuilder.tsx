...
export function TypedFilterBuilder({ ast, onApply, onClear, scope, token, tenant, upstreams, externalChips = [], disabled = false }: {
  ...
  return <div className="typed-filter-builder">
    ...
    {open && <section ref={panel} popover="auto" style={position} className="typed-filter-dialog" role="dialog" aria-modal="false" aria-label={t('filter.title')} onToggle={(event) => { if (event.target === event.currentTarget && event.newState === 'closed') setOpen(false); }}>
