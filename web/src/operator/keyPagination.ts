...
export function keyListPath(tenant: string, cursor?: KeyListCursor, filters?: { search?: string; status?: string; source?: string }) {
  ...
  if (filters?.source && filters.source !== 'all') query.set('creation_source', filters.source);
  if (cursor) {
