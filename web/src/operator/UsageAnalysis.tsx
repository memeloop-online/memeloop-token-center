...
function ChartCard({ title, children, className = '', onSelect, points, timeZone }: { title: string; children: ReactNode; className?: string; onSelect?: (point: UsageAnalysisTimeBucket) => void; points: UsageAnalysisTimeBucket[]; timeZone: string }) {
  ...
  return <article className={`panel usage-chart-card ${className}`}><div className="panel-title"><h2>{title}</h2><span>{timeZone}</span></div>{points.length === 0 ? <div className="empty">{localCopy[locale].chartEmpty}</div> : <>{children}<ChartTable onSelect={onSelect} points={points} timeZone={timeZone} /></>}</article>;
}
...
export function UsageAnalysis({ token, tenant, upstreams, onOpenSession }: { token: string; tenant: string; upstreams: UpstreamAccount[]; onOpenSession: (session: UsageAnalysisSessionBucket) => void }) {
  ...
  const chartFormatters: UsageChartFormatters = useMemo(() => ({ bucket: (epoch) => new Date(epoch).toLocaleString(locale === 'en' ? 'en-US' : 'zh-CN', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: false, timeZone: stats?.time_zone ?? 'UTC' }), cost: (value, currency) => formatCurrency(value, currency, locale), duration: (value) => formatMilliseconds(value, locale), number: (value) => formatMetricDisplay(value, locale).text, percent: (value) => formatPercent(value, locale) }), [locale, stats?.time_zone]);
  ...
  const reportedRange = stats
    ? `${new Date(stats.from_created_at).toLocaleString(locale === 'en' ? 'en-US' : 'zh-CN', { timeZone: stats.time_zone })} – ${new Date(stats.to_created_at).toLocaleString(locale === 'en' ? 'en-US' : 'zh-CN', { timeZone: stats.time_zone })}`
    : undefined;

  return <div className="usage-page"><div className="usage-heading"><div><h2>{t('usage.title')}</h2><p className="muted">{t('usage.description')}</p>{stats && <span className="usage-time-zone" title={reportedRange}>{stats.time_zone}</span>}</div><button type="button" className="secondary" disabled={loading || !token.trim()} onClick={() => setRefresh((value) => value + 1)}>{loading ? t('common.loading') : t('usage.refresh')}</button></div>
