...
export function UsageAnalysis({ token, tenant, upstreams, onOpenSession }: { token: string; tenant: string; upstreams: UpstreamAccount[]; onOpenSession: (session: UsageAnalysisSessionBucket) => void }) {
  ...
  return <div className="usage-page">
    ...
    {stats && <section className="usage-tab-panel" role="tabpanel">
      ...
      {tab === 'sessions' && <><p className="usage-dimension-contract">{t('usage.sessionGrouping')} {t('usage.sessionScope')}</p><DimensionTable title={t('usage.sessions')} values={stats.by_session} labelForValue={(bucket) => bucket.unlinked || bucket.id.startsWith('unlinked:') ? t('sessions.unlinkedRequests') : bucket.label} onSelect={onOpenSession} /></>}
