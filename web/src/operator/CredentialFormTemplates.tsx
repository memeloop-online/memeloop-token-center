...
function CredentialObjectTemplate(props: ObjectFieldTemplateProps) {
  ...
  if (props.fieldPathId.path.length === 0 && props.schema.properties?.policy) {
    ...
    return <div className="credential-form-journey">
      ...
      <AdvancedFormSection title={copy.policy} description={copy.policyHint} invalid={Boolean(props.errorSchema?.policy) || funding.some(field => Boolean(props.errorSchema?.[field.name]))}>
  ...
  if (props.schema.properties?.enforcement_mode) {
    ...
    return <FormSection title={copy.policy} description={copy.policyHint}>
