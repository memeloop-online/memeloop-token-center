...
test('Overview keeps current sections visible through independent endpoint failure, late tenant data, and responsive trend charts', { timeout: 45_000 }, async () => {
  ...
  try {
    ...
    const successRateMetric = metricCards.nth(3);
    assert.equal(await successRateMetric.locator('.metric-label').textContent(), 'Success rate');
    assert.equal(await successRateMetric.locator('[data-ratio]').getAttribute('data-ratio'), String(15 / 17), 'success-rate background uses actual summary counts');
    ...
    assert.equal(await trendData.getByRole('columnheader', { name: /Local settlement/ }).count(), 1);
