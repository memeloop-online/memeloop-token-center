...
test('quota reads share ownership, bounded batches, partial failures and credit expiry', { timeout: 60_000 }, async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('[data-summary]').filter({ hasText: 'Waiting to refresh' }).count(), 2);
    assert.equal(await page.getByRole('button', { name: 'Refresh all upstream quotas', exact: true }).isDisabled(), true);
    await page.evaluate(() => { window.releaseQuota(0); window.releaseQuota(1); window.releaseQuota(2, 503); });
    await page.waitForFunction(() => window.quotaReadCalls.length === 5);
    await page.evaluate(() => { window.releaseQuota(3); window.releaseQuota(4); });
    await page.getByText('5/5', { exact: true }).waitFor();
    assert.equal(await page.evaluate(() => window.quotaPeak), 3);
