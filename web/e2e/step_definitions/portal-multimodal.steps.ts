...
Then('自助门户显示自己的 51 条请求和分页统计', async function (this: DogfoodWorld) {
  ...
  await assertExactText(metric(page, '词元用量'), '600');
...
When('管理员通过可见生成任务页查看排队任务详情并取消', async function (this: DogfoodWorld) {
  ...
  const drawerCancel = page.locator('.drawer').getByRole('button', { name: '取消', exact: true });
  await drawerCancel.click();
  const confirmation = page.getByRole('dialog', { name: '请确认操作', exact: true });
  await assertVisible(confirmation);
  assert.equal(await confirmation.evaluate(element => !!element.closest('.drawer-owned-portals') && !element.closest('[inert], [aria-hidden="true"]')), true, 'confirmation stays in its initiating drawer, not the isolated page');
  assert.equal(await panel.evaluate(element => !!element.closest('[inert]')), true, 'background remains isolated while nested confirmation is active');
  await confirmation.getByRole('button', { name: '取消', exact: true }).focus();
  await page.keyboard.press('Shift+Tab');
  assert.equal(await confirmation.getByRole('button', { name: '确认继续', exact: true }).evaluate(element => document.activeElement === element), true);
  await page.keyboard.press('Escape');
  await confirmation.waitFor({ state: 'detached' });
  assert.equal(await page.locator('.drawer').count(), 1, 'confirmation Escape must not close the parent drawer');
  assert.equal(await drawerCancel.evaluate(element => document.activeElement === element), true, 'cancel returns focus to the initiating drawer action');
  await drawerCancel.click();
  await assertVisible(confirmation);
...
Then('控制台使用双游标只补齐缺失请求且正常关闭和切页均不报错', async function (this: DogfoodWorld) {
  ...
  const alternateTenant = (await tenantPicker.locator('option').evaluateAll(
