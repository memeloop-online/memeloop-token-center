...
- `MTC_LIVE_PROVIDER_SECRET_CANARY`: an exact, non-production, 16-128 character URL-safe sentinel stored in a dedicated disabled provider credential. The test asserts that it is absent from provider/request list APIs, request detail APIs, and the rendered DOM.
...
Both URL values must be bare, distinct HTTPS origins: paths, queries, fragments, embedded credentials, and HTTP are rejected before Chromium starts. Use a dedicated disabled provider for the canary; never replace this value with a real provider secret.
