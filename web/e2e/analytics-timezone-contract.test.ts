...
declare global { interface Window { timeFixture: { epoch: number; chartLabels: () => string[] | undefined; todayQuery: () => string | undefined } } }
test('UTC bucket epochs display in the browser zone while local day queries keep their real boundaries', { timeout: 30000 }, async context => {
 ...
 try{
  for(const timezoneId of ['Asia/Shanghai','UTC']){
   ...
   await page.locator('.usage-chart-table summary').first().click();
