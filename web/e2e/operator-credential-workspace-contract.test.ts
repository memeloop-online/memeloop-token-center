...
test('credential workspaces isolate loads and preserve issued service plaintext', { timeout: 60_000 }, async () => {
  ...
  try {
    ...
    for (const locale of ['en', 'zh-CN'] as const) {
      ...
      assert.equal(await existingRouting.locator('.selection-chip button').isEnabled(), true, 'the existing disabled grant remains explicitly removable');
      ...
      assert.equal(await client.getByRole('option').count(), 0, 'unselected disabled routes are absent, not a wall of unusable search results');
      ...
      assert.match(await routes.evaluate(input => document.getElementById(input.getAttribute('aria-activedescendant')!)?.textContent ?? ''), /Team Kimi/, 'ArrowUp reaches the last available account route');
      await routes.press('ArrowDown');
      assert.match(await routes.evaluate(input => document.getElementById(input.getAttribute('aria-activedescendant')!)?.textContent ?? ''), /Team Kimi/, 'ArrowDown stays on an available account route');
