...
function response(overrides: Partial<ManagedModelSyncResponse['routes']> = {}): ManagedModelSyncResponse {
  return {
    catalog: {
      ...
      error_code: null, models: [{ id: 'model-a', protocol: 'openai' }], disabled_models: [],
    },
...
test('catalog protocols map onto route protocols with an openai fallback', () => {
  ...
  assert.equal(inferManagedRouteProtocol('vendor-specific'), 'openai');
});
