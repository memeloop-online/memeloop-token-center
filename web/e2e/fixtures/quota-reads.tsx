...
function Fixture() {
  ...
  return <main><button onClick={() => void quota.readAll()} disabled={quota.progress?.busy}>{t('quota.refreshAll')}</button><button onClick={() => { void quota.read(accounts[0]); void quota.read(accounts[0]); }}>Duplicate read</button><button onClick={() => setGeneration(value => value + 1)}>Change generation</button><button onClick={() => setTenant('other')}>Change tenant</button><button onClick={() => setDisabled(['account-1'])}>Disable second</button><output>{quota.progress ? `${quota.progress.done}/${quota.progress.total}` : 'idle'}</output>
