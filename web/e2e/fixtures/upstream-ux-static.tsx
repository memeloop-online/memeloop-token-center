...
const snapshot: UpstreamQuotaSnapshot = {
  ...
  reset_capability: { provider_supported: true, implementation_available: true, prepare_available: true, available_credits: 2, applicable_credits: 1, reason: null, credit_error_code: null }, error_code: null,
};
function Preview() {
  const { t } = useI18n();
  ...
  return <main className="main"><article className="panel"><h1>Static quota acceptance · no network</h1><section className="upstream-quota"><h2>{t('quota.title')}</h2><p className="notice error" role="alert">{t('quota.refreshFailedRetained')}</p><UpstreamQuotaDetails snapshot={snapshot} /><details className="upstream-danger-zone" open><summary>{t('quota.resetAction')}</summary><p>{t('quota.resetWarning')}</p><UpstreamQuotaReset accountId="mock-account" accountName="Mock Codex" tenant="default" token="mock-only" snapshot={snapshot} /></details></section>{confirmationDialog}</article></main>;
}
