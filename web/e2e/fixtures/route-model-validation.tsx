...
function Fixture() {
  ...
  return <main style={{ padding: 12 }}>
    ...
    <UpstreamModelCombobox token="fixture" tenant="fixture" accountIds={[account]} includedProviderGroupIds={[]} excludedProviderGroupIds={[]} syncAccountIds={[account]} protocol={new URLSearchParams(location.search).get('protocol') || 'openai'} value={value} onChange={setValue} customModelConfirmed={new URLSearchParams(location.search).get('confirmed') !== 'false'} onValidityChange={(next, allowed) => { setValid(next); setCustom(allowed); }} />
