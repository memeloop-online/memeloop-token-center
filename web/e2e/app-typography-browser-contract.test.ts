...
test('production AppShell typography and actions share Fluent tokens in both themes and sizes', { timeout: 90_000 }, async () => {
  ...
  try {
    ...
    const requests = [base,
      { ...base, request_id: 'synthetic-success', created_at: start + 30_000, completed_at: start + 50_000, status_code: 200, duration_ms: 20_000, input_tokens: 200, output_tokens: 100 },
      { ...base, request_id: 'synthetic-failure', created_at: start + 60_000, completed_at: start + 90_000, status_code: 502, duration_ms: 30_000, input_tokens: 40, output_tokens: 10 },
    ];
    ...
    await totalTokens.locator('.metric-value', { hasText: '500' }).waitFor();
