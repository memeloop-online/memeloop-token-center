...
test('route list exposes group-only candidate scope and readable models without changing routing', { timeout: 45_000 }, async () => {
  ...
  try {
    ...
    for (const locale of ['zh-CN', 'en']) {
      ...
      await rows.filter({ hasText: 'Kimi mixed' }).getByRole('button', { name: locale === 'en' ? '1 candidate accounts' : '1 个候选账号', exact: true }).click();
      await page.getByRole('tooltip').waitFor();
      assert.match(await page.getByRole('tooltip').innerText(), /团队排除组/);
      assert.doesNotMatch(await page.getByRole('tooltip').innerText(), /Kimi team account/, 'the list must not re-expand excluded members beyond the server candidate set');
      ...
      for (const [width, theme] of [[390, 'light'], [1440, 'dark']] as const) {
        ...
        await page.keyboard.press('Escape'); await range.tap(); await tip.waitFor();
