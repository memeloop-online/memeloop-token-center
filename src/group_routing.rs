...
pub(crate) async fn prepare(
    ...
    for (index, candidate) in candidates.iter().enumerate() {
        if let Some(binding) = state
            .db
            .candidate_group_strategy(tenant_id, candidate.route_id, candidate.account_id)
            .await?
        {
            buckets
                .entry((std::cmp::Reverse(binding.priority), binding.id))
                .or_default()
                .push((index, candidate.clone(), binding.strategy, binding.version));
        }
    }
    ...
    for ((_, group_id), members) in buckets {
        ...
        match result {
            Ok(mut plan) => {
                ...
                plan.candidates.sort_by_key(|directive| {
...
pub(crate) async fn observe(
    ...
    .await
    {
        ...
        _ => {
            tracing::warn!(upstream_account_id=%account,stage="group_routing_observe_fallback","group observe failed; native health policy retained");
            None
        }
    }
}
