use std::{
    collections::BTreeMap,
    path::{Component, PathBuf},
    str::FromStr,
    time::{Duration, Instant},
};

#[cfg(test)]
use std::future::Future;

use axum::{
    Json, Router,
    body::{Body, Bytes},
    extract::{DefaultBodyLimit, MatchedPath, Path, Query, Request, State},
    http::{HeaderMap, HeaderValue, StatusCode, header},
    middleware::{self, Next},
    response::{IntoResponse, Response},
    routing::{get, patch, post, put},
};
use base64::{Engine as _, engine::general_purpose::STANDARD};
use futures_util::StreamExt;
use rust_decimal::Decimal;
use serde::Deserialize;
use serde_json::{Value, json};
use tokio_stream::wrappers::ReceiverStream;
use tower::limit::ConcurrencyLimitLayer;
use tower_http::{
    request_id::{MakeRequestUuid, PropagateRequestIdLayer, SetRequestIdLayer},
    trace::TraceLayer,
};
use uuid::Uuid;

mod audio;
mod auth;
mod billing;
mod cloud_entitlements;
mod cloud_principals;
mod control_requests;
mod credentials;
mod diagnostics;
mod filter_assistant;
mod generation;
mod generation_quarantine;
mod groups;
mod health;
mod image_generation_quarantine;
mod kimi_transport;
mod limits;
mod model_picker;
mod model_routes;
mod monitoring_snapshot;
pub(crate) mod plugin_execution;
mod plugins;
mod proxy;
pub(crate) use proxy::{MediaAttemptBudget, wait_media_recovery};
pub(crate) use proxy::{
    MediaAttemptGuard, MediaAttemptTerminal, classify_media_rate_limit,
    current_gateway_failure_domain,
};
mod proxy_diagnostics;
mod request_detail;
mod request_normalization;
mod responses_via_chat;
mod router;
mod routes;
mod routing;
mod self_service;
mod sessions;
mod sse;
mod tenants;
mod traffic;
mod upstreams;
mod usage_analysis;
mod web;

use audio::create_audio_transcription;
use auth::{
    admit_cloud_webhook_before_body, authenticate_control_before_body, authenticate_downstream,
    authenticate_gateway_before_body, authenticated_service, management_tenant,
    require_global_service, require_service, require_service_any, require_service_tenant,
};
use billing::*;
use cloud_entitlements::{
    list_memeloop_cloud_subscription_events, self_memeloop_cloud_entitlements,
    sync_memeloop_cloud_subscription,
};
use cloud_principals::ensure_memeloop_cloud_principal;
use control_requests::{
    ManagementTenantQuery, cancel_internal_generation, configuration_schemas,
    filter_assistant_billing_choices, get_filter_assistant_settings, get_filter_presets,
    internal_generation, internal_generation_asset, internal_generations,
    internal_request_archive_content, internal_request_asset, internal_request_detail,
    internal_request_events, internal_requests, internal_stats, list_tenants,
    plan_filter_with_assistant, plugin_manifests, provider_types, put_filter_assistant_settings,
    put_filter_preset, typed_internal_requests,
};
use credentials::*;
#[cfg(target_os = "linux")]
use diagnostics::cpu_profile;
#[cfg(all(not(target_env = "msvc"), not(target_env = "musl")))]
use diagnostics::heap_profile;
use diagnostics::runtime_diagnostics;
use generation::*;
use groups::*;
use health::{
    deprecated_health, liveness, observe_http, prometheus_metrics, readiness, security_headers,
    version,
};
use limits::*;
use model_picker::list_model_picker_options;
use model_routes::{
    archive_model_route, create_model_route, delete_model_route, get_archived_model_route,
    list_model_routes, retire_model_route_upstreams, set_model_route_enabled, update_model_route,
};
use plugins::{get_plugin_configuration, get_plugin_service_data, put_plugin_configuration};
use request_detail::*;
pub use router::{router, router_for_role};
use routes::{control_router, gateway_router};
use routing::*;
use self_service::*;
use sessions::*;
use tenants::{
    archive_tenant, create_tenant, delete_tenant, list_tenant_management, rename_tenant,
    restore_tenant,
};
use traffic::{
    AppliedTraffic, Protocol, TrafficPolicyProtocols,
    apply_traffic_plugin_for_existing_idempotency, apply_traffic_policy,
    authorize_applied_traffic_policy, component_provider_timeout, component_provider_url,
    inject_controlled_output_ceiling, negotiate_openai_responses_websocket,
    normalize_component_provider, prepare_component_provider, proxy_anthropic,
    proxy_anthropic_count_tokens, proxy_openai_chat, proxy_openai_embeddings,
    proxy_openai_responses,
};
use upstreams::*;
use web::{operator_index, portal_index, web_asset};

pub(crate) use upstreams::{
    refresh_managed_upstream_oauth_for_worker, trigger_copilot_remint_on_auth_failure,
};

use crate::{
    AppState,
    config::RuntimeRole,
    db::{
        ApplyCloudEntitlementInput, AttachGenerationJobResult, CancelEntitlementInput,
        CloudRoutingGrantSnapshot, CloudSubscriptionEventInput, CreateGenerationJobResult,
        CreateKeyInput, CreateServiceTokenInput, CreateUpstreamAccountInput, EntitlementOperation,
        FinishMeteredSynchronousRequest, FinishProxyRequest, FinishProxyRequestResult,
        FinishSynchronousImageRequest, FinishSynchronousImageResult, GenerationJobIdempotency,
        ProxyConversationInput, ReauthorizeUpstreamAccountInput, ReconcileEntitlementInput,
        ReplaceEntitlementInput, RequestListFilter, RouteSelectionOptions, StartGenerationJobInput,
        StartMeteredSynchronousRequest, StartProxyRequest, StartSynchronousImageRequest,
        StartSynchronousImageResult, StatsFilter, SynchronousImageIdempotencyClaim,
        UpdateUpstreamAccountInput, unix_millis,
    },
    error::AppError,
    model::{AuthenticatedKey, AuthenticatedService, KeyPolicy, KeyPolicyInput, TokenUsage},
    network::{self, OutboundScope},
    oauth::{
        CursorOAuthEndpoints, CursorPollResult, OAuthReauthorizationTarget, StartCursorLogin,
        codex_device::{
            CodexDevicePollResult, CodexDevicePollScope, OAUTH_DRIVER as CODEX_OAUTH_DRIVER,
            PROVIDER_DRIVER as CODEX_PROVIDER_DRIVER, StartCodexDeviceLogin,
            poll_codex_device_login, start_codex_device_login,
        },
        poll_cursor_login, refresh_cursor_credential, refresh_managed_oauth_credential,
        resolve_managed_oauth_refresh_adapter, start_cursor_login,
    },
    plugin::{PreparedProviderRequest, memeloop::token_center::types::RequestContext},
    provider::{ResolvedUpstream, UpstreamCredential, validate_config},
    proxy_lifecycle::{
        MAX_DOWNSTREAM_SEND_WAIT, MAX_PROXY_LIFETIME, MAX_PROXY_STREAM_LIFETIME,
        abandon_proxy_archive_attempt, confirm_proxy_delivery_with_retry,
        finish_proxy_request_with_retry, prepare_proxy_delivery_with_retry,
        response_archive_requires_cleanup,
    },
};

#[cfg(test)]
use crate::db::CreateModelRouteInput;

#[cfg(test)]
mod tests;
