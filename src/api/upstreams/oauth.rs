...
pub(crate) async fn refresh_managed_upstream_oauth(
    ...
    refresh_managed_upstream_oauth_impl(state, account_id, idempotency_key, None).await
}
...
pub(crate) async fn refresh_managed_upstream_oauth_for_worker(
    ...
    account_id: Uuid,
    idempotency_key: &str,
    blocking: &crate::worker::BlockingTasks,
) -> Result<crate::provider::UpstreamAccountView, AppError> {
    refresh_managed_upstream_oauth_impl(state, account_id, idempotency_key, Some(blocking)).await
}
...
async fn refresh_managed_upstream_oauth_impl(
    ...
    let refreshed: Result<UpstreamCredential, AppError> = async {
        ...
        let refreshed = match driver.as_str() {
            crate::oauth::authorization_code::FLOW => {
                let adapter = state.providers.get(&account.driver).and_then(|provider| provider.oauth_adapter.as_ref())
                    .ok_or_else(|| AppError::Conflict("OAuth provider contribution is unavailable".into()))?;
                crate::oauth::authorization_code::refresh(&state.http, &credential, adapter, unix_millis(), state.config.allow_oauth_loopback, &request_guard).await?
            }
    ...
    if let Some(blocking) = blocking {
        ...
        super::models::sync_upstream_models_after_refresh(state, account.id, Some(blocking)).await;
    } else {
        super::trigger_upstream_model_sync(state.clone(), account.id);
    }
    Ok(account)
...
fn supports_oauth_refresh_proxy(driver: &str) -> bool {
    matches!(driver, crate::oauth::codex_device::OAUTH_DRIVER | crate::oauth::authorization_code::FLOW)
}
