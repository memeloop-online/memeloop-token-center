...
async fn image_arm_rejects_staging_paths_without_matching_bound_request_receipt() {
    for corruption in [
        ...
        sqlx::query(&format!("CREATE TRIGGER invalidate_request_receipt AFTER UPDATE OF state ON archive_staging_attempts WHEN NEW.state = 'bound' AND NEW.purpose = 'request' BEGIN UPDATE archive_staging_attempts SET {corruption} WHERE attempt_id = NEW.attempt_id; END"))
            .execute(&pool).await.unwrap();
