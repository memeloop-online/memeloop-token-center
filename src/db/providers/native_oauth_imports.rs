...
impl Database {
    ...
    pub async fn import_native_kimi_oauth_cohort(
        ...
        for (_, _, input) in &prepared {
            ...
            if row.try_get::<String, _>("cohort_id")?
                != existing_cohort_id.as_deref().ok_or_else(|| {
                    AppError::Conflict(
                        "native Kimi OAuth receipt has no matching cohort".into(),
                    )
                })?
            {
                return Err(AppError::Conflict(
        ...
        let allowed = matches!((existing_count, changed_count), (0, 0) | (1, 0) | (2, 0) | (2, 2));
        ...
        let (cohort_id, cohort_updated_at) = if let Some(row) = cohort {
            (row.try_get::<String, _>("id")?, row.try_get::<i64, _>("updated_at")?)
        } else {
        ...
        let mut created = 0;
        let mut rotated = 0;
        for (index, (config_json, credential_ciphertext, input)) in
...
fn validate_existing_kimi_account(
    ...
    if input.expected_current_account_id.as_deref() != Some(account_id.as_str())
        ...
        || row.try_get::<Option<String>, _>("oauth_session_id")?.as_deref()
