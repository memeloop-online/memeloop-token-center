...
pub(crate) async fn publish_text_settlement_in_transaction(
    ...
    let row = sqlx::query(
        "SELECT r.id AS request_id, CASE WHEN r.protocol = 'audio-transcription' THEN 'generation' ELSE 'text' END AS request_kind, u.account_id, r.key_id, r.model, r.cost_micros, r.completed_at, CASE WHEN r.protocol = 'audio-transcription' THEN NULL ELSE r.input_tokens END AS input_tokens, CASE WHEN r.protocol = 'audio-transcription' THEN NULL ELSE r.cached_input_tokens END AS cached_input_tokens, CASE WHEN r.protocol = 'audio-transcription' THEN NULL ELSE r.cache_write_tokens END AS cache_write_tokens, CASE WHEN r.protocol = 'audio-transcription' THEN NULL ELSE r.output_tokens END AS output_tokens, CASE WHEN r.protocol = 'audio-transcription' THEN NULL ELSE r.usage_basis END AS usage_basis, u.enforcement_mode, l.id AS settlement_id, l.amount_micros, l.currency, l.created_at AS settled_at FROM request_records r JOIN usage_reservations u ON u.id = r.reservation_id LEFT JOIN ledger_entries l ON l.account_id = u.account_id AND l.key_id = u.key_id AND l.kind = 'usage' AND l.source = u.id WHERE r.id = $1 AND r.completed_at IS NOT NULL",
    )
