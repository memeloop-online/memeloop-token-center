...
impl Database {
    ...
    pub async fn request_archive_refs(
        ...
        if let Some(locator) = locator.filter(|locator| locator.key_id == key_id.to_string()) {
            let row = sqlx::query(
                "SELECT r.id, r.created_at, r.completed_at, CAST(NULL AS BIGINT) AS source_completed_at, r.protocol, r.model, r.upstream_account_id, r.model_route_id AS route_id, r.status_code, CAST(NULL AS TEXT) AS generation_status, r.duration_ms, r.first_output_ms, r.generation_duration_ms, r.usage_basis, CASE WHEN r.completed_at IS NULL THEN NULL ELSE r.input_tokens END AS input_tokens, CASE WHEN r.completed_at IS NULL THEN NULL ELSE r.cached_input_tokens END AS cached_input_tokens, CASE WHEN r.completed_at IS NULL THEN NULL ELSE r.cache_write_tokens END AS cache_write_tokens, CASE WHEN r.completed_at IS NULL THEN NULL ELSE r.output_tokens END AS output_tokens, CASE WHEN r.completed_at IS NULL THEN NULL ELSE r.billed_units END AS billed_units, CASE WHEN r.completed_at IS NULL THEN NULL ELSE NULLIF(r.billing_unit, '') END AS billing_unit, CASE WHEN r.completed_at IS NULL THEN NULL ELSE r.cost_micros END AS cost_micros, CASE WHEN r.completed_at IS NULL THEN NULL ELSE NULLIF(r.currency, '') END AS currency, 1 AS billable, r.error_code, CASE WHEN request_spool.state = 'uploading' OR spool.state = 'uploading' THEN 'uploading' WHEN request_spool.state = 'pending' OR spool.state = 'pending' THEN 'pending' WHEN request_spool.state = 'capturing' OR spool.state = 'capturing' OR r.completed_at IS NULL THEN 'capturing' WHEN request_spool.state IS NULL AND spool.state IS NULL AND r.request_object LIKE 'metadata-only-json:%' AND (r.response_object LIKE 'metadata-only-json:%' OR r.response_object LIKE 'provider-reference-json:%') THEN 'metadata_only' WHEN request_spool.state = 'gap' OR spool.state = 'gap' OR r.request_object LIKE 'gap://%' OR r.response_object IS NULL OR r.response_object LIKE 'gap://%' THEN 'gap' ELSE 'bound' END AS archive_state, CASE WHEN request_spool.state IN ('capturing', 'pending', 'uploading') OR spool.state IN ('capturing', 'pending', 'uploading') OR r.completed_at IS NULL THEN NULL WHEN request_spool.state = 'gap' THEN COALESCE(request_spool.gap_reason, request_spool.last_error_code, 'archive_object_unavailable') WHEN spool.state = 'gap' THEN COALESCE(spool.last_error_code, 'archive_object_unavailable') WHEN r.request_object LIKE 'gap://%' OR r.response_object IS NULL OR r.response_object LIKE 'gap://%' THEN 'archive_object_unavailable' ELSE NULL END AS archive_reason, request_spool.state AS request_spool_state, COALESCE(request_spool.gap_reason, request_spool.last_error_code) AS request_spool_reason, spool.state AS response_spool_state, spool.last_error_code AS response_spool_reason, r.request_object, r.response_object, r.conversation_cluster_id AS session_id, CASE WHEN r.conversation_cluster_id IS NULL THEN 'unlinked' ELSE 'confirmed' END AS session_association, observation.compaction, observation.session_name, observation.task_kind, observation.agent_id, observation.metadata_source AS semantics_source, CAST(NULL AS TEXT) AS tenant_external_id, CAST(NULL AS TEXT) AS credential_key_id, CAST(NULL AS TEXT) AS key_alias, CAST(NULL AS TEXT) AS principal_external_id FROM request_records r LEFT JOIN conversation_observations observation ON observation.request_id = r.id AND observation.key_id = r.key_id AND observation.cluster_id = r.conversation_cluster_id LEFT JOIN request_archive_spools request_spool ON request_spool.request_id = r.id AND request_spool.tenant_id = r.tenant_id AND request_spool.reservation_id = r.reservation_id LEFT JOIN response_archive_spools spool ON spool.request_id = r.id AND spool.tenant_id = r.tenant_id AND spool.reservation_id = r.reservation_id WHERE r.id = $1 AND r.created_at = $2 AND r.key_id = $3",
            )
            .bind(&request_id)
    ...
    async fn session_archive_unlinked_refs_global(
        ...
        let row = sqlx::query(
            r#"SELECT u.archive_request_id AS id,
                      ...
                      u.protocol, u.model, u.status_code, u.duration_ms, CAST(NULL AS BIGINT) AS first_output_ms, CAST(NULL AS BIGINT) AS generation_duration_ms, CAST(NULL AS TEXT) AS usage_basis,
                      ...
                      observation.compaction, observation.session_name, observation.task_kind,
...
fn build_request_list_query(
    ...
    let mut query = PortableRequestListQuery::new(
        "SELECT id, created_at, completed_at, source_completed_at, protocol, model, upstream_account_id, route_id, status_code, generation_status, duration_ms, first_output_ms, generation_duration_ms, usage_basis, input_tokens, cached_input_tokens, cache_write_tokens, output_tokens, billed_units, billing_unit, cost_micros, currency, billable, error_code, archive_state, archive_reason, session_id, session_association, compaction, session_name, task_kind, agent_id, semantics_source, tenant_external_id, credential_key_id, key_alias, principal_external_id FROM (SELECT * FROM (SELECT r.id, r.created_at, r.completed_at, CAST(NULL AS BIGINT) AS source_completed_at, r.protocol, r.model, r.upstream_account_id, r.model_route_id AS route_id, r.status_code, CAST(NULL AS TEXT) AS generation_status, r.duration_ms, r.first_output_ms, r.generation_duration_ms, r.usage_basis, CASE WHEN r.completed_at IS NULL THEN NULL ELSE r.input_tokens END AS input_tokens, CASE WHEN r.completed_at IS NULL THEN NULL ELSE r.cached_input_tokens END AS cached_input_tokens, CASE WHEN r.completed_at IS NULL THEN NULL ELSE r.cache_write_tokens END AS cache_write_tokens, CASE WHEN r.completed_at IS NULL THEN NULL ELSE r.output_tokens END AS output_tokens, CASE WHEN r.completed_at IS NULL THEN NULL ELSE r.billed_units END AS billed_units, CASE WHEN r.completed_at IS NULL THEN NULL ELSE NULLIF(r.billing_unit, '') END AS billing_unit, CASE WHEN r.completed_at IS NULL THEN NULL ELSE r.cost_micros END AS cost_micros, CASE WHEN r.completed_at IS NULL THEN NULL ELSE NULLIF(r.currency, '') END AS currency, CAST(1 AS BIGINT) AS billable, r.error_code, CASE WHEN request_spool.state = 'uploading' OR spool.state = 'uploading' THEN 'uploading' WHEN request_spool.state = 'pending' OR spool.state = 'pending' THEN 'pending' WHEN request_spool.state = 'capturing' OR spool.state = 'capturing' OR r.completed_at IS NULL THEN 'capturing' WHEN request_spool.state IS NULL AND spool.state IS NULL AND r.request_object LIKE 'metadata-only-json:%' AND (r.response_object LIKE 'metadata-only-json:%' OR r.response_object LIKE 'provider-reference-json:%') THEN 'metadata_only' WHEN request_spool.state = 'gap' OR spool.state = 'gap' OR r.request_object LIKE 'gap://%' OR r.response_object IS NULL OR r.response_object LIKE 'gap://%' THEN 'gap' ELSE 'bound' END AS archive_state, CASE WHEN request_spool.state IN ('capturing', 'pending', 'uploading') OR spool.state IN ('capturing', 'pending', 'uploading') OR r.completed_at IS NULL THEN NULL WHEN request_spool.state = 'gap' THEN COALESCE(request_spool.gap_reason, request_spool.last_error_code, 'archive_object_unavailable') WHEN spool.state = 'gap' THEN COALESCE(spool.last_error_code, 'archive_object_unavailable') WHEN r.request_object LIKE 'gap://%' OR r.response_object IS NULL OR r.response_object LIKE 'gap://%' THEN 'archive_object_unavailable' ELSE NULL END AS archive_reason, r.conversation_cluster_id AS session_id, CASE WHEN r.conversation_cluster_id IS NULL THEN 'unlinked' ELSE 'confirmed' END AS session_association, observation.compaction, observation.session_name, observation.task_kind, observation.agent_id, observation.metadata_source AS semantics_source",
    );
...
fn push_identity_eligibility(
    ...
    match source_alias {
        "r" | "g" => {
            ...
            query.push(source_alias);
            query.push(".tenant_id) OR EXISTS (SELECT 1 FROM usage_reservations identity_reservation JOIN credit_accounts identity_account ON identity_account.id = identity_reservation.account_id WHERE identity_reservation.id = ");
            query.push(source_alias);
            query.push(".reservation_id AND identity_reservation.key_id = ");
            query.push(source_alias);
            query.push(".key_id AND identity_account.tenant_id = ");
            query.push(source_alias);
            query.push(".tenant_id))");
        }
        "u" => {
...
fn request_view_from_row(row: &AnyRow) -> Result<RequestView, AppError> {
    ...
    let generation = (generation_status.is_some() || audio_transcription).then_some(
        RequestGenerationUsageView {
            billed_units,
            billing_unit,
        },
    );
...
SELECT e.*, COALESCE(r.created_at, g.created_at) AS created_at,
       ...
       r.first_output_ms, r.generation_duration_ms, r.usage_basis,
       ...
       COALESCE(facts.billed_units, CASE WHEN r.completed_at IS NOT NULL THEN r.billed_units ELSE NULL END) AS billed_units,
       COALESCE(NULLIF(facts.billing_unit, ''), CASE WHEN r.completed_at IS NOT NULL THEN NULLIF(r.billing_unit, '') ELSE NULL END) AS billing_unit,
       ...
       CASE WHEN g.id IS NOT NULL THEN
            CASE WHEN g.status IN ('preparing', 'queued') THEN 'pending'
                 ...
                 ELSE 'bound' END
            ELSE CASE WHEN request_spool.state = 'uploading' OR spool.state = 'uploading' THEN 'uploading' WHEN request_spool.state = 'pending' OR spool.state = 'pending' THEN 'pending' WHEN request_spool.state = 'capturing' OR spool.state = 'capturing' OR r.completed_at IS NULL THEN 'capturing' WHEN request_spool.state IS NULL AND spool.state IS NULL AND r.request_object LIKE 'metadata-only-json:%' AND (r.response_object LIKE 'metadata-only-json:%' OR r.response_object LIKE 'provider-reference-json:%') THEN 'metadata_only' WHEN request_spool.state = 'gap' OR spool.state = 'gap' OR r.request_object LIKE 'gap://%' OR r.response_object IS NULL OR r.response_object LIKE 'gap://%' THEN 'gap' ELSE 'bound' END
...
fn request_archive_refs_from_row(row: AnyRow) -> Result<RequestArchiveRefs, AppError> {
    ...
    let projection = |prefix: &str, locator: Option<&str>| -> Result<_, AppError> {
        ...
        if let Some(state) = spool_state {
            let state = request_archive_state(state)?;
            let reason = if state == RequestArchiveState::Gap {
                row.try_get::<Option<String>, _>(format!("{prefix}_spool_reason").as_str())?
                    .or_else(|| Some("archive_object_unavailable".to_owned()))
            } else {
                None
            };
            Ok((state, reason))
        } else if locator.is_none() && completed_at.is_none() {
            Ok((RequestArchiveState::Capturing, None))
        } else {
            Ok(locator_archive_projection(locator))
        }
    };
...
fn locator_archive_projection(location: Option<&str>) -> (RequestArchiveState, Option<String>) {
    ...
    } else if location.is_some_and(|location| !location.starts_with("gap://")) {
