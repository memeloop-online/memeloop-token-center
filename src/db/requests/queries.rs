    ...
    async fn session_archive_unlinked_refs_global(
        ...
        let row = sqlx::query(
            r#"SELECT u.archive_request_id AS id,
                      ...
                      u.protocol, u.model, u.status_code, u.duration_ms, CAST(NULL AS BIGINT) AS first_output_ms, CAST(NULL AS BIGINT) AS generation_duration_ms,
...
SELECT e.*, COALESCE(r.created_at, g.created_at) AS created_at,
       ...
       r.first_output_ms, r.generation_duration_ms,
       ...
       CASE WHEN g.id IS NOT NULL THEN
            CASE WHEN g.status IN ('preparing', 'queued') THEN 'pending'
                 ...
                 ELSE 'bound' END
            ELSE CASE WHEN request_spool.state = 'uploading' OR spool.state = 'uploading' THEN 'uploading' WHEN request_spool.state = 'pending' OR spool.state = 'pending' THEN 'pending' WHEN request_spool.state = 'capturing' OR spool.state = 'capturing' OR r.completed_at IS NULL THEN 'capturing' WHEN request_spool.state = 'gap' OR spool.state = 'gap' OR r.request_object LIKE 'gap://%' OR r.response_object IS NULL OR r.response_object LIKE 'gap://%' THEN 'gap' ELSE 'bound' END
