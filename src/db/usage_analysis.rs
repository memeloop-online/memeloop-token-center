...
impl Database {
    ...
    async fn aggregate_usage_analysis(
        ...
        for row in generation_dimension_rows {
            ...
            let id: String = row.try_get("dimension_id")?;
            let currency: String = row.try_get("currency")?;
            let units: i64 = row.try_get("units")?;
            match dimension.as_str() {
                "modality" => {
                    generation_units_by_modality.push(UsageAnalysisGenerationUnitsByModality {
                        modality: id,
                        currency,
                        units,
                    })
                }
                "billing_unit" => generation_units_by_billing_unit.push(
                    UsageAnalysisGenerationUnitsByBillingUnit {
                        billing_unit: id,
                        currency,
                        units,
                    },
        ...
        generation_units_by_modality.sort_by(|left, right| {
            left.modality
                .cmp(&right.modality)
                .then_with(|| left.currency.cmp(&right.currency))
        });
        generation_units_by_billing_unit.sort_by(|left, right| {
            left.billing_unit
                .cmp(&right.billing_unit)
                .then_with(|| left.currency.cmp(&right.currency))
        });
        ...
        Ok(UsageAnalysisResponse {
            ...
            summary,
            generation_units_by_modality,
            generation_units_by_billing_unit,
            time_series,
            by_model,
            by_key,
            by_session,
...
fn usage_analysis_generation_fact_sql(
    ...
    format!(
        r#"SELECT f.tenant_id, f.key_id,
                  ...
                  f.upstream_account_id, f.model_route_id, f.currency,
                  CAST(1 AS BIGINT) AS requests,
