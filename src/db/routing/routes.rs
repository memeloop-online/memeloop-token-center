...
use super::create_idempotency::{
    RouteCreateClaim, RouteCreateFingerprintInput, claim_route_create, find_owned_route_create,
    request_fingerprint,
};
use super::equivalence::{
    find_equivalent_route_in_transaction, route_relations_match_in_transaction, same_ids,
};
use super::grant_revisions::{
    compare_and_bump_route_grant_revision, lock_routing_relation_writes, route_grant_revision,
};
use super::input::{
    bounded_group_names, bounded_unique_ids, create_or_find_route_groups,
    resolve_existing_route_group_names, validate_route_fields,
};
use super::types::{
    CreateRoutedModelRouteInput, CreateRoutedModelRouteResult, ReplaceRouteRoutingInput,
    RouteCreateDisposition, RouteCreateIdempotencyKey, RouteRoutingView,
...
impl Database {
    ...
    pub async fn create_routed_model_route(
        ...
        let result = self
            .create_routed_model_route_with_idempotency(input, None)
            .await?;
        Ok((result.route, result.routing))
    }

    /// Returns an exact owned replay before reading mutable route dependencies.
    ///
    /// The caller already authenticated a `routes:write` service credential
    /// and enforced its tenant boundary. Keeping this lookup separate lets the
    /// HTTP layer avoid revalidating an upstream or provider registry that may
    /// have changed since the original successful create.
    pub async fn replay_routed_model_route_if_owned(
        &self,
        input: &CreateRoutedModelRouteInput,
        idempotency_key: &RouteCreateIdempotencyKey,
    ) -> Result<Option<CreateRoutedModelRouteResult>, AppError> {
        validate_route_fields(
            &input.public_model,
            &input.upstream_model,
            &input.protocol,
        ...
        let upstream_ids =
            bounded_unique_ids(input.upstream_account_ids.clone(), "upstream accounts")?;
        let included = bounded_unique_ids(
            input.included_provider_group_ids.clone(),
            "included provider groups",
        )?;
        let excluded = bounded_unique_ids(
            input.excluded_provider_group_ids.clone(),
            "excluded provider groups",
        )?;
        let route_groups = bounded_unique_ids(input.route_group_ids.clone(), "route groups")?;
        let credential_ids =
            bounded_unique_ids(input.granted_credential_ids.clone(), "credentials")?;
        if upstream_ids.is_empty() && included.is_empty() {
            return Err(AppError::BadRequest(
                "a route requires an explicit upstream account or included provider group".into(),
            ));
        }
        let route_group_names = bounded_group_names(input.route_group_names.clone())?;
        let mut tx = self.begin_write_transaction().await?;
        let tenant_id = tenant_id(&mut tx, &input.tenant_external_id).await?;
        lock_routing_relation_writes(&mut tx, &tenant_id).await?;
        let fingerprint = request_fingerprint(RouteCreateFingerprintInput {
            tenant_id: &tenant_id,
            ...
            granted_credential_ids: &credential_ids,
        })?;
        let Some(route_id) = find_owned_route_create(
            &mut tx,
            &tenant_id,
            idempotency_key,
            &fingerprint,
        ...
        let (route, routing) = self
            .load_routed_model_route(route_id, &input.tenant_external_id)
            .await?;
        Ok(Some(CreateRoutedModelRouteResult {
            route,
            routing,
            disposition: RouteCreateDisposition::Reused,
        }))
    }

    /// Creates a route under an optional explicit ownership claim.
    ///
    /// Header-less callers retain the pre-existing semantic-equality replay
    /// path. New automation must supply a `RouteCreateIdempotencyKey` and
    /// accept only `Created` or `Reused` dispositions as ownership proof.
    pub async fn create_routed_model_route_with_idempotency(
        &self,
        input: CreateRoutedModelRouteInput,
        idempotency_key: Option<RouteCreateIdempotencyKey>,
    ) -> Result<CreateRoutedModelRouteResult, AppError> {
        validate_route_fields(
            &input.public_model,
            &input.upstream_model,
            &input.protocol,
        ...
        let request_fingerprint = idempotency_key
            .as_ref()
            .map(|_| {
                request_fingerprint(RouteCreateFingerprintInput {
                    tenant_id: &tenant_id,
                    public_model: &input.public_model,
                    upstream_model: &input.upstream_model,
                    protocol: &input.protocol,
        ...
        if let Some(idempotency_key) = idempotency_key.as_ref() {
            let fingerprint = request_fingerprint.as_deref().ok_or(AppError::Internal)?;
            if let Some(existing_route_id) =
                find_owned_route_create(&mut tx, &tenant_id, idempotency_key, fingerprint, now)
                    .await?
            {
                tx.commit().await?;
                let (route, routing) = self
                    .load_routed_model_route(existing_route_id, &input.tenant_external_id)
                    .await?;
                return Ok(CreateRoutedModelRouteResult {
                    route,
                    routing,
                    disposition: RouteCreateDisposition::Reused,
                });
        ...
        .await?;
        route_groups = bounded_unique_ids(route_groups, "route groups")?;
        let disposition = if idempotency_key.is_some() {
            RouteCreateDisposition::Created
        } else if let Some(existing) = find_equivalent_route_in_transaction(
            &mut tx,
            &tenant_id,
        ...
        .await?
        {
            ...
            return Ok(CreateRoutedModelRouteResult {
                route: existing,
                routing,
                disposition: RouteCreateDisposition::EquivalentUnowned,
            });
        } else {
            RouteCreateDisposition::Created
        };
        if let Some(idempotency_key) = idempotency_key.as_ref() {
            ...
            .await?
            {
                RouteCreateClaim::Claimed => {}
                RouteCreateClaim::Reused(existing_route_id) => {
                    tx.commit().await?;
                    let (route, routing) = self
                        .load_routed_model_route(existing_route_id, &input.tenant_external_id)
                        .await?;
        ...
        let routing = self.route_routing_view(route_id, &tenant_id, now).await?;
        Ok(CreateRoutedModelRouteResult {
            route,
            routing,
            disposition,
        })
    ...
    async fn load_routed_model_route(
        &self,
        route_id: Uuid,
        tenant_external_id: &str,
    ) -> Result<(ModelRouteView, RouteRoutingView), AppError> {
        let row = sqlx::query(
            "SELECT r.tenant_id, r.public_model, r.upstream_account_id, r.upstream_model, \
        ...
        let routing = self
            .route_routing_view(route_id, &tenant_id, updated_at)
            .await?;
        Ok((route, routing))
    }
