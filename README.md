...
Cucumber 端到端测试会启动 SQLite、内存对象存储和 Mock 上游。当前覆盖 39 个默认场景、218 个步骤，包括稳定凭据轮换、CPA 原凭据兼容、权限/额度/限流、跨租户授权矩阵、订阅 entitlement/grant、OpenAI/Anthropic/兼容客户端、API/OAuth 同管线、Cursor PKCE、Copilot/Cursor subscription bridge、CPA 账号安全导入、逻辑会话，以及 Seedance、ComfyUI、OpenAI Images、Codex Responses 生图的转发、归档和计费。`MTC_TEST_POSTGRES_URL` 会额外启用真实 PostgreSQL 数据库集成测试；v1–v23 已在 fresh restricted PostgreSQL 数据库验证 23 个迁移无缺口，locator、v22 预算并发和 v23 生成聚合也通过真实 PostgreSQL gate。CPAMP 首次、重复、迟到重叠和新水位导入另由隔离 PostgreSQL acceptance 脚本验证，不能等同于 live 数据迁移。
...
数据库 DDL 位于 `migrations/`，当前工作树 schema 版本为 v23；每个版本都在同一事务和 PostgreSQL advisory transaction lock 下应用。PostgreSQL 的请求与事件表按天分区，v21 的非分区 locator 为全局请求/事件 ID 提供唯一所有权和叶分区时间坐标；v22 用事务化状态、UTC 日桶和滚动周边界明细避免预算热路径扫描完整账本；v23 将终态生成统计写入 facts/日聚合，普通统计只在边界日读取精确 facts，耗时、费用和 pending 筛选使用最大 93 天的有界索引回退。SQLite 保留等价的小型测试 schema。连接池默认每进程最多 4 个连接且不预热，HTTP 上游连接池也有严格空闲上限；Codex 生图响应限制为 16 MiB、每副本最多两个并发缓冲，其他大对象继续流式归档，避免重现 CPA 的高内存占用。v1–v23 的隔离 PostgreSQL 功能门禁已通过；141k 行历史迁移锁时和导入生产规模数据上的 EXPLAIN/延迟仍是部署前独立性能证据，不能由功能门禁代替。
...
`ops/import-cpa-session-archive.sh` 在 CPAMP 身份导入后接入 `cpa-session-archive` v0.7.x schema-v2 JSONL，把精确匹配的请求/响应正文写入 BLAKE3 CAS，并补充可证明的会话观察。它默认只 dry-run，整批预检失败即零写入，支持 tenant/source checkpoint 和重放，且不会覆盖已有真实对象；当前通过的是 SQLite/CAS fixture，尚不是 PostgreSQL/S3 或 live session 迁移证据。
