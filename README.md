...
`POST /internal/v1/imports/cpa/managed-oauth` 使用服务端 catalog 中受审核的适配器，把 Codex 与 legacy Gemini auth 文件导入统一的托管 OAuth 提供商；调用者不能指定 driver、base URL、刷新地址或账号配置，原文件也不会写入响应或日志。Copilot/Cursor 的 opaque handle 不会被当作凭据或伪造为上游，只会进入需要原生重新授权的迁移清单，并以非秘密的稳定源 ID 保留历史关联。只有具备专用 transport 的 driver 才会对外声明可路由协议；未知的 Kimi 等私有 OAuth 文件继续 fail closed，不能把 token 文件机械伪装成通用 OAuth。
