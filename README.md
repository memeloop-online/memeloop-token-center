...
同一个镜像可按不同运行角色启动。生产部署的边界和发布前置条件由受控运维文档定义；本项目概览不记录环境拓扑或访问路径。
...
数据库 DDL 位于 `migrations/`，当前工作树 schema 版本为 v59；每个版本都在同一事务和 PostgreSQL advisory transaction lock 下应用。PostgreSQL 的请求与事件表按天分区，v21 的非分区 locator 为全局请求/事件 ID 提供唯一所有权和叶分区时间坐标；v22 用事务化状态、UTC 日桶和滚动周边界明细避免预算热路径扫描完整账本；v23–v27 增加生成与请求 facts/日聚合、按凭据分页的会话投影、生成资产元数据和 CPAMP 来源摘要；v28–v35 增加可证明的 session-archive exact/unlinked 投影、生成 worker 持久状态、稳定凭据会话修复、上游 OAuth 生命周期、CPA 原凭据映射和归档 staging fence；v36–v42 增加归档隔离区、MemeLoop Cloud entitlement 事件、凭据/账本与控制面 keyset 分页、多币种可观测性、生成取消及插件配置；v43–v48 增加提供商组、路由组、仅用于列表分类的凭据组、归一化路由授权、上游模型目录、原生 Codex OAuth 会话、目录约束、异步生成路由快照和路由关系 CAS revision；v49–v51 增加 Cloud 事件查询索引、多模态生成用量维度和逻辑会话用量汇总；v52–v53 移除旧模型白名单写入入口，并持久化 Claude Code、Copilot 和 Cursor 的独立 OAuth 流类型；v54 补齐会话维度中的缓存读写 Token 和异步多模态生成用量；v55 增加明确上报的 session/trace/span/代理/任务语义元信息，无法可靠关联的生成任务保留在稳定的 `unlinked:<key_id>` 会话；v56–v58 增加可连续验证的 session-archive schema-v2 稳定快照、删除墓碑、不可变隔离证据版本，以及单事务有界 staging；v59 为全租户请求/生成历史的模型筛选增加有序 Top-N 索引。SQLite 保留等价的小型测试 schema。v59 的 PostgreSQL 索引由事务迁移建立，在大型历史表上存在写冲突锁风险；上线前必须通过写屏障停写，或预先并发构建并验证所有分区叶索引及生成索引，不能靠提高迁移超时掩盖风险。连接池默认每进程最多 4 个连接且不预热，HTTP 上游连接池也有严格空闲上限；同步生图响应限制为 16 MiB、每副本最多两个并发缓冲，其他大对象继续流式归档。fresh PostgreSQL 16/17 的 v1–v42 历史门禁、fresh PostgreSQL 17 的 v1–v51 门禁以及当前真实迁移二进制在 PostgreSQL 17 上的完整 v1→v59 门禁均已通过；隔离的 PostgreSQL 17 快照还完成了 v58→v59 并发预构建、实际迁移与 309,888 行查询计划门禁。生产目标上的并发写入观察、最终镜像内存门禁和 live 部署仍属于发布门禁，不能用 SQLite/Cucumber 结果替代。

`node ops/migrate-cpamp.ts` 是 PostgreSQL 增量导入器：首次导入 CPAMP usage/alias，并只把 CPAMP price 当作每条历史记录的计费证据，绝不改写 Token Center 的全局价格表；之后从 checkpoint watermark 回看默认 24 小时，只把尚未存在的 event hash 写入请求明细、facts、请求日聚合及用量小时/日/会话聚合。导入行会保留归一化总输入、缓存读/写、requested/resolved/pricing model、有效服务档位、context/service 价格规则及完整定价配置摘要；缺少来源价格或归一化维度时整批失败，不会把未知值伪装成零。聚合只从本次成功取得 locator 的新 facts 产生，因此完整重放不会重复累计。旧导入行的缓存/费用修复必须使用独立的、先 plan 后显式确认的 v2 correction，详见 `docs/operations/cpamp-cache-pricing-correction.md`；它不触碰余额、账本或全局价格。回看窗口用于覆盖 CPAMP 延迟刷盘；确定性 UUID 与幂等插入保证周末切换前可以反复运行。`ops/kubernetes/cpamp-import-job.yaml` 提供了只读挂载 CPA Manager Plus PVC 的 K8s Job 模板，默认就是增量模式。`CPAMP_RESET_IMPORT=true` 仅用于重建 dogfood 导入租户，不应在生产增量切换中使用。

`node ops/import-cpa-session-archive.ts` 在 CPAMP 身份导入后接入 `cpa-session-archive` v0.7.x 真实导出的 schema-v1 或 identity-aware schema-v2 JSONL，把精确匹配的请求/响应正文写入 BLAKE3 CAS，并补充可证明的会话观察。它默认只 dry-run，整批预检失败即零写入，支持 tenant/source checkpoint 和重放，且不会覆盖已有真实对象；当前通过的是 SQLite/CAS fixture，尚不是 PostgreSQL/S3 或 live session 迁移证据。
