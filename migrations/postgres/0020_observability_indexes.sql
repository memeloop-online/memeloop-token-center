-- Match the bounded, keyset-paginated operator diagnostics queries.
...
CREATE INDEX IF NOT EXISTS key_records_tenant_alias_prefix_idx
    ON key_records (tenant_id, LOWER(alias) text_pattern_ops, id);
CREATE INDEX IF NOT EXISTS principals_tenant_external_prefix_idx
    ON principals (tenant_id, LOWER(external_id) text_pattern_ops, id);
