...
#[tokio::test]
async fn attaching_and_replaying_legacy_secret_preserves_identity_history_policy_and_balance() {
    ...
    let legacy = "fixture-only-unchanged-cpa-client-key-0001";
    let source_hash = format!("{:x}", Sha256::digest(legacy.as_bytes()));
    let attach_body = json!({"credential": legacy, "source_hash": source_hash});
    let endpoint = format!("/internal/v1/keys/{}/legacy-credentials", issued.key_id);
    let (first_status, first_attach) = request(
        &state,
        "POST",
        &endpoint,
        "test-service-token",
    ...
    .await;
    let (replay_status, replay_attach) = request(
        &state,
        "POST",
        &endpoint,
        "test-service-token",
    ...
    assert_eq!(first_attach, replay_attach);
    assert_eq!(first_attach["key_id"], issued.key_id.to_string());
    assert_eq!(
        first_attach["generation"],
        before_key["credential_generation"]
    );

    let (status, legacy_key) = request(&state, "GET", "/self/v1/key", legacy, None).await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(legacy_key, before_key);
    assert_eq!(legacy_key["key_id"], issued.key_id.to_string());
    assert_eq!(
        legacy_key["policy"],
        serde_json::to_value(expected_policy).unwrap()
    );
    assert_eq!(legacy_key["available_balance"], "10");

    let (status, legacy_stats) = request(&state, "GET", "/self/v1/stats", legacy, None).await;
    assert_eq!(status, StatusCode::OK);
    assert_eq!(legacy_stats, before_stats);
    assert_eq!(legacy_stats["summary"]["total_requests"], 1);
    assert_eq!(legacy_stats["summary"]["input_tokens"], 11);
    assert_eq!(legacy_stats["summary"]["output_tokens"], 7);

    let (status, requests) = request(&state, "GET", "/self/v1/requests", legacy, None).await;
    assert_eq!(status, StatusCode::OK);
