...
export function validateProductContracts(document: Obj): void {
  ...
  const audio = operationAt(document, "post", "/v1/audio/transcriptions"); if (!isDeepStrictEqual(audio.security, [{ clientBearer: [] }])) throw new ContractFailure("POST /v1/audio/transcriptions must retain client credential security"); if (audio.requestBody?.content?.["multipart/form-data"]?.schema?.$ref !== "#/components/schemas/OpenAIAudioTranscriptionRequest") throw new ContractFailure("audio transcription must retain its multipart request contract"); if (!subset(["200", "400", "401", "403", "408", "413", "415", "424", "429", "502", "503"], Object.keys(audio.responses ?? {}))) throw new ContractFailure("audio transcription response contract regressed"); if (audio.responses?.["200"]?.headers?.["X-MTC-Request-Id"]?.$ref !== "#/components/headers/RequestId") throw new ContractFailure("audio transcription success lost its request ID"); const audioRequest = schemas.OpenAIAudioTranscriptionRequest ?? {}; if (audioRequest.additionalProperties !== false || !setEqual(audioRequest.required ?? [], ["file", "model"])) throw new ContractFailure("audio transcription request schema must remain closed with file/model required"); if (!isDeepStrictEqual(audioRequest.properties?.response_format?.enum, ["json", "verbose_json"])) throw new ContractFailure("audio transcription response formats changed"); if (audioRequest.properties?.temperature?.minimum !== 0 || audioRequest.properties?.temperature?.maximum !== 1) throw new ContractFailure("audio transcription temperature must remain bounded from 0 to 1"); const verbose = schemas.OpenAIAudioTranscriptionVerboseJson ?? {}; if (!verbose.properties?.task || !verbose.properties?.language || !verbose.properties?.words || !verbose.properties?.segments?.items?.properties?.tokens) throw new ContractFailure("verbose audio transcription fields regressed");
  const routeProtocols = ["openai", "anthropic", "openai-audio", "generation"]; for (const name of ["CreateModelRouteRequest", "ReplaceModelRouteRequest", "ModelRoute"]) if (!isDeepStrictEqual(schemas[name]?.properties?.protocol?.enum, routeProtocols)) throw new ContractFailure(`${name} route protocol contract changed`); if (!isDeepStrictEqual(schemas.ProviderType?.properties?.protocols?.items?.enum, routeProtocols)) throw new ContractFailure("provider protocol contract changed");
  ...
  const requiredIdempotency = { namespace: "stable-key-id", fingerprint: "post-policy-canonical-model-and-request", "completed-replay": "conflict-without-upstream-resubmission", "different-payload-status": 400, "in-progress-status": 409, "expired-pending": "atomically-reclaimed", "persisted-url-results": "protected-provider-reference-only" }; if (!object(idempotency) || Object.entries(requiredIdempotency).some(([key, value]) => idempotency[key] !== value)) throw new ContractFailure("POST /v1/images/generations idempotency contract changed"); if (!setEqual(idempotency["completed-replay-effects"] ?? [], ["no-request-record", "no-upstream-request", "no-quota-reservation", "no-usage-settlement", "no-key-rate-limit-window-consumption", "no-image-execution-permit"])) throw new ContractFailure("completed synchronous image replay acquired side effects"); if (!setEqual(idempotency["replay-preconditions"] ?? [], ["authentication", "traffic-policy-evaluation"])) throw new ContractFailure("synchronous image replay preconditions changed"); if (!setEqual(idempotency["fresh-execution-only"] ?? [], ["route-resolution", "price-lookup", "outbound-client-validation"])) throw new ContractFailure("completed image replay regained mutable route/price dependencies");
  const ensureCloudPrincipal = operationAt(document, "post", "/internal/v1/integrations/memeloop-cloud/principals/ensure"); if (!isDeepStrictEqual(ensureCloudPrincipal.security, [{ serviceBearer: [] }]) || ensureCloudPrincipal["x-required-scope"] !== "keys:write") throw new ContractFailure("MemeLoop Cloud principal ensure lost tenant-scoped key-write authentication"); if (ensureCloudPrincipal.requestBody?.content?.["application/json"]?.schema?.$ref !== "#/components/schemas/MemeLoopCloudPrincipalEnsureRequest") throw new ContractFailure("MemeLoop Cloud principal ensure request schema changed"); if (ensureCloudPrincipal.responses?.["200"]?.content?.["application/json"]?.schema?.$ref !== "#/components/schemas/ProvisionedCloudCredential") throw new ContractFailure("MemeLoop Cloud principal ensure credential result changed"); const ensureSchema = schemas.MemeLoopCloudPrincipalEnsureRequest ?? {}; if (ensureSchema.additionalProperties !== false || !setEqual(ensureSchema.required ?? [], ["tenant_external_id", "principal_external_id", "currency"])) throw new ContractFailure("MemeLoop Cloud principal ensure identity contract changed");
...
function matchingRule(path: string, boundary: Obj): Obj { const matches = boundary.rules.filter((rule: Obj) => !(rule.excluded_paths ?? []).includes(path) && ((rule.exact_paths ?? []).includes(path) || (rule.path_prefixes ?? []).some((prefix: string) => path.startsWith(prefix)))); if (matches.length !== 1) throw new ContractFailure(`route ${path} matched ${matches.length} boundary rules: ${JSON.stringify(matches.map((rule: Obj) => rule.name ?? "<unnamed>"))}`); return matches[0]!; }

export function checkContract(source: Obj[], specRoutes: Obj[], spec: Obj, boundary: Obj): Obj {
  validateProductContracts(spec); const responsesBoundary = matchingRule("/v1/responses", boundary); if (responsesBoundary.name !== "responses-compatible-api" || responsesBoundary.transport_contract !== "POST is the HTTP proxy; GET temporarily returns authenticated 426 negotiation until native WebSocket support replaces that handler") throw new ContractFailure("Responses GET/POST transport boundary is incomplete"); const sourceMap = new Map(source.map((route) => [`${route.method} ${route.path}`, route])); const specMap = new Map(specRoutes.map((route) => [`${route.method} ${route.path}`, route])); const contract: Obj[] = [];
