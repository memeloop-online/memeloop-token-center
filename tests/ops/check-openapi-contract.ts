...
export function validateProductContracts(document: Obj): void {
  ...
  const batchCatalog = operationAt(document, "post", "/internal/v1/upstream-models/query"); if (!isDeepStrictEqual(batchCatalog.security, [{ serviceBearer: [] }]) || batchCatalog["x-required-scope"] !== "routes:read") throw new ContractFailure("batch upstream model query authorization changed"); if (batchCatalog.requestBody?.content?.["application/json"]?.schema?.$ref !== "#/components/schemas/UpstreamModelBatchQueryRequest") throw new ContractFailure("batch upstream model query lost its JSON request schema"); if (batchCatalog.responses?.["200"]?.content?.["application/json"]?.schema?.$ref !== "#/components/schemas/AggregatedUpstreamModelCatalog") throw new ContractFailure("batch upstream model query response diverged from the GET catalog query"); const batchCatalogRequest = schemas.UpstreamModelBatchQueryRequest ?? {}; if (batchCatalogRequest.additionalProperties !== false || !setEqual(batchCatalogRequest.required ?? [], ["tenant_external_id", "account_ids"])) throw new ContractFailure("batch upstream model query must require only tenant and explicit account selection"); const batchCatalogProperties = batchCatalogRequest.properties ?? {}; if (batchCatalogProperties.account_ids?.maxItems !== 500 || batchCatalogProperties.include_provider_group_ids?.maxItems !== 100 || batchCatalogProperties.exclude_provider_group_ids?.maxItems !== 100 || batchCatalogProperties.q?.maxLength !== 500 || batchCatalogProperties.limit?.maximum !== 200) throw new ContractFailure("batch upstream model query bounds changed");
