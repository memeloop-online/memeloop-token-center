...
test("audio transcription stays multipart, bounded, and request-id correlated", () => {
  const document = cloneDocument(); const operation = document.paths["/v1/audio/transcriptions"].post; assert.deepEqual(operation.security, [{ clientBearer: [] }]); assert.equal(operation.requestBody.content["multipart/form-data"].schema.$ref, "#/components/schemas/OpenAIAudioTranscriptionRequest"); assert.equal(operation.responses["200"].headers["X-MTC-Request-Id"].$ref, "#/components/headers/RequestId"); assert.deepEqual(Object.keys(operation.responses), ["200", "400", "401", "403", "408", "413", "415", "424", "429", "502", "503"]); const request = document.components.schemas.OpenAIAudioTranscriptionRequest; assert.deepEqual(request.required, ["file", "model"]); assert.deepEqual(request.properties.response_format.enum, ["json", "verbose_json"]); assert.deepEqual([request.properties.temperature.minimum, request.properties.temperature.maximum], [0, 1]); const verbose = document.components.schemas.OpenAIAudioTranscriptionVerboseJson; assert.ok(verbose.properties.task && verbose.properties.language && verbose.properties.words); assert.ok(verbose.properties.segments.items.properties.tokens); validateProductContracts(document);
  delete request.properties.temperature.maximum; assert.throws(() => validateProductContracts(document), /temperature must remain bounded/u);
});
test("audio routing uses the dedicated openai-audio protocol", () => { const document = cloneDocument(); const expected = ["openai", "anthropic", "openai-audio", "generation"]; for (const name of ["CreateModelRouteRequest", "ReplaceModelRouteRequest", "ModelRoute"]) assert.deepEqual(document.components.schemas[name].properties.protocol.enum, expected); assert.deepEqual(document.components.schemas.ProviderType.properties.protocols.items.enum, expected); validateProductContracts(document); });
...
test("usage analysis contract is currency safe and canonical", () => {
  ...
  assert.deepEqual(parameters.granularity.schema.enum, ["auto", "hour", "day"]); assert.deepEqual(parameters.protocol.schema.enum, ["openai", "anthropic", "openai-image", "audio-transcription", "generation"]); assert.deepEqual(parameters.status.schema.enum, ["success", "error"]); assert.deepEqual(parameters.upstream_account_id.schema.oneOf, [{ type: "string", format: "uuid" }, { type: "string", const: "unassigned" }]); assert.deepEqual(document.components.parameters.UpstreamAccountFilter.schema, { type: "string", format: "uuid" });
...
test("settlement adjustments use a dedicated forward-only capability", () => {
  const document = cloneDocument(); const operation = document.paths["/internal/v1/accounts/{account_id}/settlements/{settlement_id}/adjustments"].put;
  assert.deepEqual(operation.security, [{ serviceBearer: [] }]);
  assert.equal(operation["x-required-scope"], "settlements:adjust");
  assert.deepEqual(operation.parameters.map((parameter: Obj) => parameter.$ref ?? parameter.name), ["#/components/parameters/AccountId", "settlement_id", "#/components/parameters/RequiredIdempotencyKey"]);
  assert.equal(operation.requestBody.content["application/json"].schema.$ref, "#/components/schemas/ReconcileSettlementAdjustmentRequest");
  assert.equal(operation.responses["200"].content["application/json"].schema.$ref, "#/components/schemas/SettlementAdjustmentResult");
  assert.equal(operation.responses["201"].content["application/json"].schema.$ref, "#/components/schemas/SettlementAdjustmentResult");
  assert.equal(operation.responses["200"].headers["Cache-Control"].schema.const, "no-store");
  assert.equal(operation.responses["201"].headers["Cache-Control"].schema.const, "no-store");
  for (const status of ["400", "401", "403", "404", "409"]) assert.ok(status in operation.responses);
  ...
  assert.deepEqual(result.properties.adjustment_entry_id.type, ["string", "null"]);
  assert.ok(document.components.schemas.ServiceScope.enum.includes("settlements:adjust"));
});
