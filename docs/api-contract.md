...
The route prefix is currently `v1`; the application and this contract are `0.1.0`, and the current working-tree database schema is v58. Fresh isolated PostgreSQL 16 and 17 databases applied migrations v1→v42 successfully, and a fresh isolated PostgreSQL 17 database applied v1→v51 with the native Codex OAuth and routing-group gates. An isolated PostgreSQL 17 copy of the current v55 schema subsequently applied v56→v58 and passed the archive advisory-lock and locator-CAS integration gate. The PostgreSQL/MinIO integration suite passed archive staging, generation, managed OAuth, proxy exactly-once, conversation pagination, entitlement, observability, archive quarantine, session-archive exact/unlinked, usage analysis and legacy-credential gates. This retained evidence does not replace the pending fresh v1→v58, production-snapshot migration lock-time, or live rollout gates. Control-plane callers with `metrics:read` can inspect `/version`, which reports build revision/target/timestamp, the current and supported API versions, compatibility text, and deprecated paths. All `/internal`, `/self`, and `/v1` responses carry `X-MTC-API-Version: v1`. The stated v1 policy permits additive changes; removals require a documented deprecation window. A release process must still update the OpenAPI version and run lint/conformance checks whenever a request, response, scope, or status code changes.
