...
`POST /internal/v1/model-routes` accepts an optional single visible-ASCII `Idempotency-Key`. With one, the server HMACs the key before storage and binds it, for 30 days, to the canonical tenant-scoped route payload and stable route ID. The initial result is `201` with `X-MTC-Route-Create-Disposition: created`; an exact retry is `200` with `...: reused`; reuse with a different payload is `409`. The raw key is never stored. It is opaque operation metadata, not a credential: callers must use one bounded identifier per logical create and must not include secrets, request content or personal data. A disabled route with an unexpired create claim cannot be deleted, so an exact retry keeps its promised stable result. Header-less callers retain equality replay (`201`, `...: equivalent_unowned`) for compatibility, but it is explicitly not ownership proof. Controlled automation must send an idempotency key and accept only `created` or `reused`; it must never adopt an arbitrary equivalent route.
