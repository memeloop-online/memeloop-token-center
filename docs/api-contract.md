...
Do not expose the control service through the public client ingress. `/livez` proves only process liveness. `/readyz` checks PostgreSQL and the configured archive with bounded, coalesced dependency probes. PostgreSQL alone determines the HTTP readiness result: a database failure or timeout returns 503, while an archive failure returns 200 with `status: "degraded"` and `checks.archive: "failed"` so an object-store outage cannot remove every gateway endpoint. Archive dependency metrics record the same failed check. This fail-open readiness decision does not make asset access fail-open: image/video archive reads, writes and finalization still fail closed when their required object-store operation cannot complete. `/healthz` is a deprecated compatibility alias for `/livez` and advertises its successor in response headers. Gateway and worker roles do not register `/metrics` or `/version`.

The machine-checked boundary table is [`../openapi/route-boundaries.json`](../openapi/route-boundaries.json). `tests/ops/check-openapi-contract.ts` extracts every Axum method/path from the common, control and gateway routers, classifies it through that table, and compares it with every OpenAPI `operationId`. It also resolves local schema references and enforces the non-regression baseline recorded in that boundary file. The only intentional source-only routes are the operator shell and wildcard UI assets; adding another undocumented exception fails CI.
...
`POST /internal/v1/model-routes` accepts an optional single visible-ASCII `Idempotency-Key`. With one, the server HMACs the key before storage and binds it, for 30 days, to the canonical tenant-scoped route payload and stable route ID. The optional `enabled` field defaults to `true` and is canonicalized with the request, so controlled preparation may create a disabled route without silently changing a retry into an enabled one. The initial result is `201` with `X-MTC-Route-Create-Disposition: created`; an exact retry is `200` with `...: reused`; reuse with a different payload is `409`. The raw key is never stored. It is opaque operation metadata, not a credential: callers must use one bounded identifier per logical create and must not include secrets, request content or personal data. A disabled route with an unexpired create claim cannot be deleted, so an exact retry keeps its promised stable result. Header-less callers retain equality replay (`201`, `...: equivalent_unowned`) for compatibility, but it is explicitly not ownership proof. Controlled automation must send an idempotency key and accept only `created` or `reused`; it must never adopt an arbitrary equivalent route.
...
SiliconFlow T2V is enabled only when an `http-json` account declares `video_api: siliconflow-v1` and the route's exact upstream model appears in the account's bounded `video_models` allowlist. The same stable account and encrypted API credential may continue to serve text and OpenAI-compatible image routes without advertising those other models as video-capable. Its base URL must end at `/v1`, and generated assets may be fetched only from the same origin or an administrator-configured exact `result_origins` entry. `POST /v1/videos/generations` accepts `input.parameters` with required `prompt` and `image_size` (`1280x720`, `720x1280`, or `960x960`) plus optional scalar `negative_prompt` and integer `seed`; all other fields, including image input, are rejected. The worker POSTs the route's upstream model and those fields to `/video/submit`, polls `/video/status` with bounded backoff, accepts only `Succeed`, `InQueue`, `InProgress`, or `Failed`, and streams the single successful `results.videos[].url` into CAS before job settlement. Because SiliconFlow does not document replay-safe submission, a lost submit outcome becomes `submission_outcome_unknown` and is never POSTed twice. Provider reason text and result URLs are not persisted or returned.
...
Before the native Responses WebSocket transport is released, an authenticated `GET /v1/responses` probe returns HTTP 426 with `Upgrade: websocket` and the stable `websocket_upgrade_required` error code. This is a temporary negotiation response that lets compatible clients immediately use the existing `POST /v1/responses` HTTP flow instead of retrying a 405 response. It does not remove the WebSocket endpoint from the product direction: native WebSocket support will replace only this GET handler while retaining the same gateway authentication and route boundary.

`DELETE /self/v1/generations/{job_id}` atomically cancels and refunds a queued job that has not acquired an active worker lease. A running Seedance or ComfyUI job is first fenced into `cancelling`, invalidating any polling lease; its reservation remains held until the worker confirms the driver-specific upstream cancellation, after which terminal settlement refunds it exactly once. Failed cancellation calls retry with bounded backoff and never cause an unconfirmed refund. SiliconFlow publishes no confirmed cancellation operation, so an already-submitted SiliconFlow job rejects cancellation and retains its reservation until normal terminal settlement. Repeating a `cancelling` or `cancelled` request is safe. A job whose submission outcome is still ambiguous, whose archived result is already being finalized, or which is otherwise terminal cannot be cancelled.
...
The route prefix is currently `v1`; the application and this contract are `0.1.0`, and the current working-tree database schema is v59. Schema v59 adds ordered indexes for global, model-filtered request and generation history so each all-tenant source can produce a bounded Top-N before merging. Fresh isolated PostgreSQL 16 and 17 databases applied migrations v1→v42 successfully, a fresh isolated PostgreSQL 17 database applied v1→v51 with the native Codex OAuth and routing-group gates, and on 2026-08-28 the actual current migration binary applied another fresh PostgreSQL 17 database through exactly 59 contiguous versions with both v59 parent indexes and all ten fresh request leaf indexes valid/ready. An isolated API2 PostgreSQL 17 snapshot clone separately passed the concurrent v59 prebuild, actual v58→v59 migration, valid/ready inventory and 309,888-row plan gates. Both v59 temporary databases were deleted after verification. The PostgreSQL/MinIO integration suite passed archive staging, generation, managed OAuth, proxy exactly-once, conversation pagination, entitlement, observability, archive quarantine, session-archive exact/unlinked, usage analysis and legacy-credential gates. PostgreSQL builds the v59 parent and generation indexes transactionally, so production operations must still either hold a write barrier or prebuild and verify equivalent indexes concurrently across every request partition before migration. This evidence does not replace writer-contention observation, final-image memory/CI or live rollout gates. Control-plane callers with `metrics:read` can inspect `/version`, which reports build revision/target/timestamp, the current and supported API versions, compatibility text, and deprecated paths. All `/internal`, `/self`, and `/v1` responses carry `X-MTC-API-Version: v1`. The stated v1 policy permits additive changes; removals require a documented deprecation window. A release process must still update the OpenAPI version and run lint/conformance checks whenever a request, response, scope, or status code changes.

## Operational metrics

`/metrics` is registered only on control/all roles and requires a service credential with `metrics:read`. It exports build identity, dependency readiness, bounded HTTP and upstream request/error/duration series, active handlers/streams/upstream exchanges, database pool and generation queue state, process CPU/RSS, jemalloc totals, component memory accounting, aggregate plugin/cache state, the active `gateway_body_reads` admission count and fixed-label `gateway_body_rejections_total` 413 counters. Gateway body admission is process-local, fails fast with HTTP 503 rather than a credential-policy 429, and releases its permit once bounded buffering ends. Ordinary gateway requests remain limited to 4 MiB; `/v1/responses` has a configurable 16 MiB default (hard-capped at 64 MiB) and an independent bounded large-body admission. Every label value comes from a fixed application enumeration; labels intentionally exclude tenant, credential, model, concrete URL/resource ID, request ID, error text and every other user-controlled value. Optional runtime, CPU and heap diagnostics are likewise control-only and `metrics:read` protected, but their routes remain unregistered unless `MTC_RUNTIME_PROFILING_ENABLED=true`; duration, concurrency and output are bounded as documented in the production operations guide.
