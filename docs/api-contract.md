...
The route prefix is currently `v1`; the application and this contract are `0.1.0`, and the current working-tree database schema is v35. A fresh isolated PostgreSQL 17 database applied migrations v1→v35 successfully. `tests/postgres.rs` passed 5/5, and focused PostgreSQL gates passed for archive staging, generation jobs, managed OAuth imports, proxy atomicity, conversation pagination, entitlements, observability filters, session-archive exact/unlinked handling, usage analysis and legacy-credential bulk import. This evidence covers the fresh migration sequence and those focused behaviors; it does not replace the pending production-size migration lock-time, final imported-scale EXPLAIN/latency, current-SHA 15-minute memory, or live rollout gates. Control-plane callers with `metrics:read` can inspect `/version`, which reports build revision/target/timestamp, the current and supported API versions, compatibility text, and deprecated paths. All `/internal`, `/self`, and `/v1` responses carry `X-MTC-API-Version: v1`. The stated v1 policy permits additive changes; removals require a documented deprecation window. A release process must still update the OpenAPI version and run lint/conformance checks whenever a request, response, scope, or status code changes.
