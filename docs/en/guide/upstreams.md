...
Codex usage/credit reads, Kimi usage reads and Antigravity's read-only `retrieveUserQuotaSummary` RPC share a bounded exponential backoff with jitter. Account config `quota_read_policy` is runtime-adjustable: `max_attempts` defaults to 3 (including the first request, range 1–4), `initial_delay_millis` defaults to 200 (50–2000), and `total_timeout_millis` defaults to 20000 (1000–30000). This is independent of inference `transport_policy.connect_attempts`; shorter explicit transport timeouts still apply. Only pre-header connection/timeout failures and HTTP 429/503 with a valid `Retry-After` can retry, within one absolute deadline. Authentication errors, body/JSON failures, empty payloads and changed/revoked credential generations do not retry. Reset consumption, OAuth and generation requests are excluded.

Responses include `attempts` with allowlisted failure stages, attempt/limit, outcome, timing and manual/bulk trigger, plus top-level `cache_hit`. These diagnostics never contain request URLs, proxy credentials, tokens or supplier error bodies. Failed reads preserve the existing bounded stale snapshot; concurrent waiters share the failure, but later clicks can retry immediately rather than reusing a negatively cached error. Cached responses retain the original supplier attempt history.
