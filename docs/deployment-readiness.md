...
- The current schema generation is **v51**. The chart renders
  `memeloop.io/schema-generation: "v51"` on its migration Job, and the Helm
...
- v49 adds MemeLoop Cloud event-query indexes, v50 adds generation modality and
  billing dimensions, and v51 adds logical-session usage rollups.
...
2. Rehearse the complete migration sequence against a fresh PostgreSQL database
   ...
   pending migration through v51 while recording lock duration.
...
4. Require the Job to finish at v51 before rolling gateway, control, or worker
